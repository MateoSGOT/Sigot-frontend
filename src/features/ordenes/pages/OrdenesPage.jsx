  ...
  return (
    <div className="progreso-container">
      ...
      <div className="progreso-actions">
        ...
        {estadoNum === 3 && (
          <p className="progreso-done">âœ“ Orden completada</p>
        )}
        {estadoNum !== 0 && (
...
export default function OrdenesPage() {
  ...
  // Mapa de repuestos por id para acceder rÃ¡pido a datos de garantÃ­a como fallback
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    if (!editForm.Diagnostico || !editForm.Kilometraje) { setEditError('DiagnÃ³stico y kilometraje son obligatorios.'); return; }
  ...
  const handleAddServicio = async (e) => {
    ...
    if (!result.error) { setAddServForm({ Id_Servicio: '', precio_unitario: '' }); setAddServError(''); }
  ...
  const handleAddRepuesto = async (e) => {
    ...
    if (!result.error) { setAddRepForm({ Id_Repuesto: '', cantidad: '', precio_unitario: '' }); setAddRepError(''); }
  ...
  const columns = [
    ...
    { key: 'Vehiculo', label: 'VehÃ­culo', render: (v, row) => <span className="font-medium">{v || row.vehiculo || row.Placa || 'â€”'}</span> },
    { key: 'Cliente', label: 'Cliente', render: (v, row) => v || row.cliente || 'â€”' },
    { key: 'Diagnostico', label: 'DiagnÃ³stico', render: v => <span className="diag-cell">{v || 'â€”'}</span> },
    { key: 'FechaIngreso', label: 'Ingreso', render: v => formatDate(v) },
    { key: 'FechaEntrega', label: 'Entrega', render: v => formatDate(v) },
    { key: 'Kilometraje', label: 'Km', render: v => v ? `${Number(v).toLocaleString('es-CO')} km` : 'â€”' },
    ...
    <div className="page">
      ...
      <div className="card">
        <div className="card__header">
          <SearchBar
            value={search}
            onChange={setSearch}
            placeholder="Buscar por vehÃ­culo, cliente, diagnÃ³stico..."
            filterSlot={
              <>
        ...
        <Table columns={columns} data={filtered} loading={loading} pageSize={pageSize} emptyMessage="No se encontraron Ã³rdenes de trabajo" />
      </div>
            ...
            <div className="orden-tabs">
              {['info', 'servicios', 'repuestos'].map(tab => (
                <button key={tab} className={`orden-tab${activeTab === tab ? ' orden-tab--active' : ''}`} onClick={() => setActiveTab(tab)}>
                  {tab === 'info' ? 'InformaciÃ³n general' : tab === 'servicios' ? 'Servicios' : 'Repuestos'}
                </button>
              ))}
            </div>
            ...
            {activeTab === 'info' && selected && (
              <div style={{ marginTop: '1rem' }}>
                <div className="detail-grid">
                  <div className="detail-item"><span className="detail-label">VehÃ­culo</span><span className="detail-value">{selected.Vehiculo || selected.Placa || 'â€”'}</span></div>
                  <div className="detail-item"><span className="detail-label">Cliente</span><span className="detail-value">{selected.Cliente || 'â€”'}</span></div>
                  ...
                  <div className="detail-item"><span className="detail-label">Kilometraje</span><span className="detail-value">{selected.Kilometraje ? `${Number(selected.Kilometraje).toLocaleString('es-CO')} km` : 'â€”'}</span></div>
                  ...
                  <div className="detail-item" style={{ gridColumn: 'span 2' }}><span className="detail-label">DiagnÃ³stico</span><span className="detail-value">{selected.Diagnostico || 'â€”'}</span></div>
                </div>
                ...
                {/* GarantÃ­as de los repuestos usados */}
                {selected?.repuestos?.some(r => {
                  ...
                  <div className="orden-garantias-section">
                    <p className="detail-label">GarantÃ­as de repuestos</p>
                      ...
                      {selected.repuestos.map((r, i) => {
                        ...
                        return (
                          <div key={i} className="orden-garantia-item">
                            <span className="orden-garantia-nombre">{r.Nombre || `Repuesto #${r.Id_Repuesto}`}</span>
                            <span className="orden-garantia-badge">âœ“ {garantia} {unidad}</span>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="orden-total-card" style={{ marginTop: '1.25rem' }}>
                  <div className="orden-total-breakdown">
                    ...
                    <div className="orden-total-row"><span>Mano de obra</span><span>{manoDeObra != null ? formatCurrency(manoDeObra) : 'â€”'}</span></div>
                  </div>
                  ...
                  {selected?.servicios?.length > 0 ? (
                    selected.servicios.map((s, i) => (
                      <div key={i} className="orden-item-row">
                        <span className="orden-item-name">{s.Nombre || s.nombre || `Servicio #${s.Id_Servicio}`}</span>
                      ...
                      return (
                        <div key={i} className="orden-item-row">
                          <div className="orden-item-name-group">
                            <span className="orden-item-name">{r.Nombre || r.nombre || `Repuesto #${r.Id_Repuesto}`}</span>
                            {garantia && (
                              <span className="orden-item-garantia">Â· GarantÃ­a: {garantia} {unidad}</span>
                            )}
                          </div>
                ...
                <div className="orden-add-form">
                  ...
                  {addRepForm.Id_Repuesto && addRepForm.precio_unitario && (
                    <p style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.375rem' }}>
                      Precio por defecto: {formatCurrency(addRepForm.precio_unitario)} â€” puedes modificarlo para esta orden.
                    </p>
                  )}
                </div>
              </div>
      ...
      >
        ...
        <form className="form-grid" onSubmit={handleEditSubmit} noValidate>
          ...
          <div className="form-group span-2"><label className="form-label">DiagnÃ³stico <span className="required">*</span></label><textarea name="Diagnostico" className="form-control" value={editForm.Diagnostico} onChange={e => setEditForm(p => ({ ...p, Diagnostico: e.target.value }))} rows={4} /></div>
        </form>
      </Modal>
    </div>
  );
